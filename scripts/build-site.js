...
const navFor = (current) =>
  categories
    .map((category) => `<a href="/${category.slug}/"${category.slug === current ? ' aria-current="page"' : ""}>${category.nav}</a>`)
    .join("");
...
const footer = (current) => `
    <footer class="site-footer">
      <div class="page footer-grid">
        ...
        <div><h3 class="mono">Open source</h3><a href="/agent/">Contribute ideas</a><a href="/motion/">Browse patterns</a></div>
      </div>
    </footer>`;
